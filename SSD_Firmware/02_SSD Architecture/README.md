...
***"소프트웨어(HIL/FTL/FIL)와 하드웨어의 연결고리"***

✏️ 기능

- HAL SSD 펌웨어에서 하드웨어(NAND, DMA, ECC 등)를 직접 제어하지 않고, 일관된 인터페이스를 통해 제어할 수 있도록 추상화한 계층

- 즉 "소프트웨어(HIL/FTL/FIL)가 하드웨어를 직접 만지지 않도록 중간에 가림막 역할"을 하는 것

- 하드웨어가 달라져도 펌웨어 전체를 바꿀 필요가 없도록 해주는 유지보수성과 확장성의 핵심

🔔 HAL이 필요한 이유

- 하드웨어 추상화 : 다양한 NAND 종류, ECC 엔진, DMA 등을 소프트웨어가 동일한 방식으로 제어 가능

- 하드웨어 교체 대응 : NAND 제조사 변경, ECC 변경 시에도 HAL만 수정하면 상위 로직은 그대로 유지

- 모듈화 설계 : SSD 펌웨어를 계층적으로 분리하여 유지보수가 쉬움

- 코어 로직 단순화 : FTL/FIL은 하드웨어 제어가 아니라 ‘요청’만 함. HAL이 실제 제어 수행

- 즉 ***HAL은 하드웨어에 직접 접근할 수 있는 API함수들을 제공***하는 것이고, NAND등 하드웨어가 바뀌면 HAL API함수의 내부 구현만 바꾸고 소프트웨어는 똑같은 방식, 이름으로 API함수를 호출해서 사용할 수 있다.
